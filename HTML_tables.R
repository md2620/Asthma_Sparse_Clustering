...
#Reading the files
df = read.csv("/rds/general/user/md2620/home/asthma/Malo/Dataframes/df_for_cluster_char_final.csv")
df2 = read.csv("/rds/general/user/md2620/home/asthma/Data/UBIOPRED_CLINICAL.csv")
...
#Get category of each variable in df
lapply(df,class)

#Transform the variables you want by location to factor variables
list_to_transform <- dplyr::select(df,337:414)
list <- names(list_to_transform)
df[,list] <- lapply(df[,list],factor)

#Check category of each variable in df
lapply(df,class)
...
names(df)[names(df) == ".UBIOPRED.Adult_Cohort_.Jan_2019..OMICS.DATA.Metabolomics.Baseline.Visit.Urine.Xanthine."] <- "Omics.Xanthine"
...
####### DROP USELESS VARIABLES FROM df ######
df <- subset(df, select = -c(X.1, Unnamed..0,Unnamed..0.1))

###### Rename X to patient in df ######
names(df)[names(df) == "X"] <- "Patient"

####### NOW MERGE df and df2 ########
df_final = merge(df,df2, by="Patient")

####### Write a csv file now that your data is ready to be analyzed
setwd("/rds/general/user/md2620/home/asthma/Malo/Dataframes")
write.csv(df_final,"df_midpoint_renamed_merger_done.csv")
...
tab <- summary(tableby(cluster ~ X.UBIOPRED.Adult_Cohort_.Jan_2019..Subject.Body.Measurements.Height..cm.., data = df_final))
tab
#Turns list into dataframe
as.data.frame(tab)

df$X.UBIOPRED.Adult_Cohort_.Jan_2019..Subject.Body.Measurements.Height..cm..
# will select the names starting with the Demographic Data ignoring whether variable name is in capital letters or not
demo_var <- tidyselect::vars_select(names(df), starts_with('X.UBIOPRED.Adult_Cohort_.Jan_2019..Demographic.Data.', ignore.case = TRUE))
demo_var

bio_var <- tidyselect::vars_select(names(df), starts_with('X.UBIOPRED.Adult_Cohort_.Jan_2019..Biomarker.Data.', ignore.case = TRUE))
bio_var
...
tab_demographics_num <- summary(tableby(cluster ~ X.UBIOPRED.Adult_Cohort_.Jan_2019..Demographic.Data.Age.
                                     , data=df, numeric.simplify=TRUE, numeric.stats=c("Nmiss", "meansd")))
...
tab_demographics_cat <- summary(tableby(cluster ~ includeNA(X.UBIOPRED.Adult_Cohort_.Jan_2019..Demographic.Data.Ethnic.origin.Father.) + 
                  ...
                  includeNA(X.UBIOPRED.Adult_Cohort_.Jan_2019..Demographic.Data.Sex.), data = df, cat.stats = "countpct"))

#If you want to sort variables by p-value
summary(sort(tab_demographics_cat, decreasing = TRUE))

#Merge 2 datasets
tab_demographics <- summary(merge(tableby(cluster ~ Demographic.Age
                                          , data=df_final, numeric.simplify=TRUE, numeric.stats=c("Nmiss", "meansd")),
                                  tableby(cluster ~ includeNA(Demographic.Highest.Level.Education) + 
                                            ...
                                            includeNA(Demographic.occupational.volunteer), data = df_final, cat.stats = "countpct"),all=TRUE))
...
tab_biomarkers <- summary(tableby(cluster ~ X.UBIOPRED.Adult_Cohort_.Jan_2019..Biomarker.Data.Baseline.Visit.Luminex.Serum.C5a.pg.ml.Luminex.serum. + 
                                    ...
                                    X.UBIOPRED.Adult_Cohort_.Jan_2019..Biomarker.Data.Baseline.Visit.Luminex.Serum.DPPIV.pg.ml.Luminex.serum., data=df, numeric.stats=c("Nmiss", "meansd"),
                                  digits=2))

tab_biomarkers
#Wite to an HTML document
write2html(tab_biomarkers, "/rds/general/user/md2620/home/asthma/Malo/bimoarkers_characteristics_by_cluster_test.html")
...
#Transform the variables you want by location to factor variables
list_to_transform_HTML <- dplyr::select(df_HTML_not_processed,337:413)
list_HTML <- names(list_to_transform_HTML)
df_HTML_not_processed[,list] <- lapply(df_HTML_not_processed[,list_HTML],factor)
...
#Create parallel coordinate chart
sapply(df_HTML_not_processed,class)

#Transform your cluster variable from integer to factor.
df$cluster <- factor(df$cluster)
ggparcoord(df,
           columns = 2:5, groupColumn = 413,order = "anyClass",
           scale="globalminmax",
           showPoints = TRUE, 
           title = "No scaling",
           alphaLines = 0.3
...
#If you only want to look at one specific group:
df_HTML_not_processed %>%
  arrange(desc(cluster)) %>%
  ggparcoord(
    columns = 1:3, groupColumn = 413, order = "anyClass",
    showPoints = TRUE, 
    title = "Original",
    alphaLines = 1
  ) + 
