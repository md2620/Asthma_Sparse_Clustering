...
write.csv(final_df_un,"/rds/general/user/md2620/home/asthma/Malo/Results/Calibration/test_df_job.csv")
...
k_max <- 4
steps_lambda_job <- seq(from = 1, to = 1.3, by = 0.1)

df_total_job <- data.frame()
for(k in 2 : k_max){
  
  for (lambda in steps_lambda_job){
    mcclain_job <- mcclain_kproto(data = final_df_un, k = k, lambda = lambda, na.rm=FALSE)
    row_test <- nrow(df_total_job)+1
    df_total_job[row_test,1] <- k
    df_total_job[row_test,2] <- lambda
    df_total_job[row_test,3] <- mcclain_job$indices[1]
    
  }
}

colnames(df_total_job) <- c("K","Lambda","Mcclain_Score")

#Save dataframe for lists
write.csv(df_total_job,"/rds/general/user/md2620/home/asthma/Malo/Results/Calibration/df_mcclain_example_job.csv")

#Run matrix from there 
# Link : 
#library(ggplot2)
#plt_mcclain_job <- ggplot(df_total_job,aes(Lambda,K))+
 # geom_tile(aes(fill=Mcclain_Score))+
  #ggtitle("Calibration of lambda coefficient and K to minimize McClain score")

#ggsave("/rds/general/user/md2620/home/asthma/Malo/Results/Calibration/mcclain_matrix_example_job.jpeg", width = 8, height = 6)